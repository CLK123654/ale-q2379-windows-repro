...
def admin(command: str) -> None:
    completed = subprocess.run(
        [os.environ["PSQL_PATH"], "--dbname", os.environ["SERVER_ADMIN_URL"], "-X", "--set", "ON_ERROR_STOP=1", "--command", command],
...
database = "tenant_access_reference"
admin(f"DROP DATABASE IF EXISTS {database} WITH (FORCE)")
admin("DROP ROLE IF EXISTS north_reader, north_editor, south_editor, east_reader, portfolio_auditor, app_reader, app_writer, report_owner, policy_owner, rls_owner")
...
command = [
    ...
    "--actor-password", "LocalActorPass2379",
]
